...
import { buildFlightPathScene, type ScenePoint } from "./flightPathGeometry";
...
function PathLine({ points }: { points: ScenePoint[] }) {
  const vertices = useMemo<[number, number, number][]>(
    () => points.map((p) => [p.x, p.y, p.z]),
    [points]
  );
  return <Line points={vertices} color="#E8A020" lineWidth={2.5} />;
}
...
export function FlightPathScene({ gpsPath, className, cameraApiRef }: FlightPathSceneProps) {
